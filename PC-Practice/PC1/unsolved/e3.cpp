...
int getDias(unsigned int, unsigned int);
...
int getDias(unsigned int mes, unsigned int anio) {
	int dias = 0;
...
void contarDias(unsigned int dia, unsigned int mes, unsigned int anio, unsigned int *diastTranscurridos, unsigned int *diasParaAnioNuevo) {
	for(int i = 1; i < mes; i++) {
		*diastTranscurridos += getDias(i, anio);
	}

	*diastTranscurridos += dia;
	*diastTranscurridos -= 1;

	*diasParaAnioNuevo += (((anio % 4 == 0) ? 366 : 365) - *diastTranscurridos);
	*diasParaAnioNuevo -= 1;
} 
